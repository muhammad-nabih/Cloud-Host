			...
			<div className='mx-auto max-w-7xl'>
				...
				<div className='mt-12 space-y-4 sm:mt-16 sm:grid sm:grid-cols-2 sm:gap-6 sm:space-y-0 lg:mx-auto lg:max-w-4xl xl:max-w-none xl:grid-cols-3'>
					{pricingPlans.map((plan) => (
						<div key={plan.name} className='divide-y divide-gray-700 rounded-lg border border-gray-700 bg-gray-900 shadow-md'>
							<div className='p-6'>
								...
								<Link href='#' className='mt-8 block w-full rounded-md border border-transparent bg-blue-600 py-2 text-center text-sm font-semibold text-white hover:bg-blue-700'>
									Get Started
								</Link>
							</div>
								...
								<ul className='mt-6 space-y-4'>
									{plan.features.map((feature, index) => (
										<li key={index} className='flex space-x-3'>
											<CiCircleCheck className={`h-5 w-5 flex-shrink-0 ${feature.included ? 'text-green-500' : 'text-gray-400'}`} />
